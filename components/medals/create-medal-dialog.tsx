...
import type { Athlete, Event, Medal } from "@/types"
...
type Props = {
  ...
  athletes: Athlete[]
  events: Event[]
  onCreate: (m: Medal) => void
}

export function CreateMedalDialog({ open, onOpenChange, athletes, events, onCreate }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const newMedal: Medal = {
      ...
      medalType: medalType as "Gold" | "Silver" | "Bronze",
      ...
      <DialogContent className="sm:max-w-[425px] rounded-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 pt-4">
          <div className="space-y-2">
            <Label htmlFor="athlete" className="text-sm font-semibold">Select Athlete</Label>
          ...
          <div className="space-y-2">
            <Label htmlFor="event" className="text-sm font-semibold">Select Event</Label>
          ...
          <div className="space-y-2">
            <Label htmlFor="medal" className="text-sm font-semibold">Medal Type</Label>
          ...
          <div className="flex justify-end gap-3 pt-4">
            <Button type="button" variant="outline" onClick={() => onOpenChange(false)} className="rounded-xl h-11 border-slate-200">Cancel</Button>
            <Button type="submit" className="bg-[#1a4cd8] hover:bg-blue-700 rounded-xl h-11 px-8">Save Award</Button>
          </div>
        </form>
      </DialogContent>
    </Dialog>
...
export default CreateMedalDialog
